"""
CGS — Correlation engine.

Merges Suricata alerts + internal analysis to make
intelligent defense decisions.

Logic:
  - Critical Suricata alert → immediate block
  - Suricata alert + internal scan detected → block
  - Suricata alert + DNS anomaly → block + sinkhole
  - Alert accumulation from an IP → progressive escalation
...
logger = logging.getLogger("cgs.correlator")
...
class Correlator:
    ...
    def __init__(self, config, alert_fn, defense_engine, threat_engine, incident_engine=None):
        ...
        threading.Thread(target=self._cleanup_loop, daemon=True,
                        name="correlator-gc").start()

    def _cleanup_loop(self):
        """Removes stale IP entries older than 1 hour."""
        while True:
            time.sleep(300)
            now = time.time()
            cutoff = now - 3600
            with self._lock:
                stale = [ip for ip, d in self._ip_events.items()
                         if d["last_seen"] and d["last_seen"] < cutoff]
                for ip in stale:
                    del self._ip_events[ip]
            if stale:
                logger.debug("Correlator cleanup: removed %d stale IPs", len(stale))
    ...
    def on_suricata_event(self, evt: SuricataEvent):
        ...
        # Store non-alerts for enrichment
    ...
    def _evaluate_response(self, evt: SuricataEvent, src_ip: str, severity: int):
        ...
        with self._lock:
            ...
            # Snapshot mutable state for safe read outside lock
            suricata_alerts = ip_data["suricata_alerts"]
            categories = set(ip_data["categories"])
        ...
        if severity <= 1 and evt.alert_action != "blocked":
            ...
            if self.incident:
                self.incident.create_incident(
                    ...
                    attacker_ip=src_ip,
                    severity=severity,
                    threat_type=evt.alert_category or "Critical alert",
        ...
        if suricata_alerts >= 3:
            ...
            try:
                ...
                logger.warning("Failed to get host risk score for %s: %s", src_ip, e)
            ...
            if internal_risk >= 30:
                ...
                if self.incident:
                    self.incident.create_incident(
                        ...
                        threat_detail=(
                            f"Suricata: {suricata_alerts} alerts, "
                            f"Internal risk: {internal_risk}/100, "
                    ...
                    self._alert(
                        ...
                        title=f"Correlation: {src_ip} multi-source threat",
                        detail=(
                            f"Suricata: {suricata_alerts} alerts, "
                            f"Internal risk: {internal_risk}/100"
                        ),
