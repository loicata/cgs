"""
Threat analysis engine — the "brain" of the SIEM.

Detections:
  1. Vertical / horizontal port scan
  2. SSH and service brute-force
  3. DNS tunnel (high entropy)
  4. C2 beaconing (regular intervals)
  5. Exfiltration (abnormal volumes)
  6. ARP spoofing (MAC change)
  7. Suspicious ports (meterpreter, IRC…)
  8. Traffic anomaly vs baseline
...
logger = logging.getLogger("cgs.analyzer")
...
class ThreatEngine:
    ...
    def __init__(self, config, alert_fn):
        ...
        self._suspicious_port_alerted: dict[tuple, float] = {}  # (src,dst,port) → last_alert_ts
    ...
    def on_event(self, evt: dict):
        ...
        with self._lock:
            try:
                ...
                logger.debug("ThreatEngine event error (%s): %s", t, e)
        ...
        if "SYN" in flags and dp in (22, 23, 3389, 5900, 3306, 1433, 445, 21):
            ...
            if len(self._bf_tr[key]) >= self.bf_th:
                ...
                self._alert(severity=1, source="analyzer", category="bruteforce",
                            ...
                            detail=f"{len(self._bf_tr[key])} attempts in {self.bf_win}s",
                            src_ip=src, dst_ip=dst)
                self._bf_tr[key].clear()

        # ── Suspicious port (with dedup cooldown) ──
        if dp in SUSPICIOUS_PORTS:
            sp_key = (src, dst, dp)
            now = time.time()
            last_alerted = self._suspicious_port_alerted.get(sp_key, 0)
            if now - last_alerted > 300:  # 5-minute cooldown per (src,dst,port)
                self._risk_d[src] += 10
                self._suspicious_port_alerted[sp_key] = now
                self._alert(severity=2, source="analyzer", category="suspicious_port",
                            title=f"Suspicious port {dp}",
    ...
    def _udp(self, e):
        ...
        if dp in SUSPICIOUS_PORTS:
            self._alert(severity=3, source="analyzer", category="suspicious_port",
                        title=f"Suspicious UDP port {dp}",
    ...
    def _dns(self, e):
        ...
        if entropy >= self.dns_th and len(query) > 30:
            ...
            self._alert(severity=2, source="analyzer", category="dns_tunnel",
                        title=f"Possible DNS tunnel from {src}",
                        detail=f"Query: {query[:80]} (entropy={entropy:.2f})",
        ...
        for p in (".onion.", ".tor2web.", ".i2p."):
            if p in query:
                ...
                self._alert(severity=1, source="analyzer", category="tor_domain",
                            title=f"Anonymous network from {src}",
    ...
    def _check_beacon(self, pair, src, dst):
        ...
        if cv < self.bcn_tol:
            ...
            self._alert(severity=1, source="analyzer", category="beaconing",
                        ...
                        detail=f"Interval={mean:.1f}s σ={sd:.1f}s CV={cv:.3f} ({len(ivs)} samples)",
                        src_ip=src, dst_ip=dst)
            ...
            for key, current in checks.items():
                ...
                if abs(z) > 3:
                    self._alert(severity=3, source="analyzer", category="anomaly",
                                title=f"Anomaly: {key}",
                                detail=f"Current={current} Baseline={stat.value:.0f}±{stat.std_dev:.0f} Z={z:.1f}")
        except Exception as e:
        ...
        for ip, total in list(self._vol_tr.items()):
            if total >= self.exfil:
                ...
                self._alert(severity=2, source="analyzer", category="exfiltration",
                            title=f"Abnormal volume from {ip}",
                            detail=f"{mb:.1f} MB transmitted", src_ip=ip)
    ...
    def _update_risk(self):
        ...
        try:
            ...
            logger.warning("Failed to flush risk score deltas: %s", e)
    ...
    def _cleanup_loop(self):
        while True:
            ...
            with self._lock:
                ...
                # Decay volume tracker instead of clearing (preserves cross-cycle exfil detection)
                for ip in list(self._vol_tr):
                    self._vol_tr[ip] = int(self._vol_tr[ip] * 0.5)
                    if self._vol_tr[ip] < 1024:
                        del self._vol_tr[ip]
                # Clean up stale suspicious port dedup entries
                cutoff = now - 600
                for k in list(self._suspicious_port_alerted):
                    if self._suspicious_port_alerted[k] < cutoff:
                        del self._suspicious_port_alerted[k]
