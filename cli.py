...
console = Console()
DCFG = "/opt/cgs/config.yaml"

@click.group()
@click.option("--config","-c", default=DCFG)
@click.pass_context
def cli(ctx, config):
    """⛨  CGS — Autonomous micro-SIEM · 100%% Python."""
...
def start(ctx):
    ...
    state_file = "/opt/cgs/.install_state"
    if os.path.exists(state_file):
        ...
        if "ROLLED_BACK" in state:
            ...
            console.print("Re-run : [bold]sudo dpkg --configure cgs[/]")
            console.print("       or : [bold]sudo apt install --fix-broken[/]")
            sys.exit(1)
...
def scan(ctx):
    ...
    disc = NetworkDiscovery(cfg, AlertEngine(cfg).fire, mac_resolver=None, identity_engine=None)
...
def status(ctx):
    ...
    s = r["system"]
    console.print(f"\n[bold cyan]  ⛨ CGS — System[/]")
    console.print(f"  CPU: {s['cpu_percent']}%  RAM: {s['memory_percent']}% ({s['mem_used_gb']}/{s['mem_total_gb']} GB)  Uptime: {s['uptime_h']}h")
    for m,d in r["disk"].items():
        ...
        console.print(f"  {m:12s} [{c}]{bar}[/] {d['used_pct']}% ({d['free_gb']} GB free)")
    console.print()
...
def alerts(ctx, limit):
    ...
    for a in Alert.select().order_by(Alert.ts.desc()).limit(limit):
        t.add_row(a.ts.strftime("%d/%m %H:%M:%S"),
                  f"[{ss.get(a.severity, 'dim')}]{sl.get(a.severity, '?   ')}[/]",
