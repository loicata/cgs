...
logger = logging.getLogger("cgs.alerts")
...
class AlertEngine:
    def __init__(self, config):
        ...
        try:
            ...
            if any(nc.get(ch, {}).get("enabled") for ch in ("slack", "teams", "telegram")):
                self._notifier = NotificationDispatcher(config)
                logger.info("NotificationDispatcher enabled")
        except Exception as e:
            logger.warning("NotificationDispatcher unavailable: %s", e)
    ...
    def fire(self, severity=3, source="system", category="", title="",
        ...
        if self._ws_cb:
            ...
            except Exception as e: logger.warning("Failed to send WebSocket callback: %s", e)
    ...
    def _dispatch(self, a):
        ...
        if ch.get("syslog",{}).get("enabled"):
            self._syslog(a)
        if self._notifier and a.severity <= 3:
            try:
                self._notifier.send(a.severity, a.title, a.detail or "",
                                    src_ip=a.src_ip or "", dst_ip=a.dst_ip or "")
            except Exception as e:
                logger.error("Notification dispatch: %s", e)

    def _email(self, a):
        try:
            c = self.cfg.get("alerts.email")
            m = MIMEText(f"[{SEV[a.severity]}] {a.source}\n{a.title}\n\n{a.detail or ''}\nSrc={a.src_ip} Dst={a.dst_ip}")
            m["Subject"] = f"[CGS][{SEV[a.severity]}] {a.title}"
    ...
    def _syslog(self, a):
        try:
            ...
            syslog.openlog("cgs", syslog.LOG_PID, syslog.LOG_LOCAL0)
            syslog.syslog(p, f"[{a.source}] {a.title}")
            syslog.closelog()
        except Exception as e: logger.warning("Failed to send syslog alert: %s", e)
