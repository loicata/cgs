"""
CyberGuard Sentinel — Network discovery.
...
logger = logging.getLogger("cyberguard.discovery")

scapy_conf.verb = 0  # silence scapy

# ── Banner signatures (pour banner grabbing TCP) ──
BANNER_SIGS = [
...
class NetworkDiscovery:
    ...
    def __init__(self, config, alert_fn, mac_resolver=None, identity_engine=None):
        ...
        # python-nmap : seulement si l'admin a choisi de le garder
    ...
    def arp_sweep(self) -> list[dict]:
        ...
        logger.info("ARP sweep sur %s", self.subnets)
        t0 = time.time()
        all_hosts = []
        ...
        logger.info("ARP : %d hosts, %d nouveaux (%.1fs)", len(all_hosts), new_count, elapsed)
        return all_hosts
    ...
    def _scapy_syn_scan(self, targets: list[str]) -> dict:
        ...
        for ip in targets:
            ...
            # Envoyer tous les SYN en batch via scapy.sr()
            ...
            for sent, rcv in answered:
                if rcv.haslayer(TCP) and rcv[TCP].flags == 0x12:  # SYN-ACK
                    ...
                    # Banner grabbing (socket TCP classique)
        ...
        logger.info("SYN scan completed : %d ports ouverts (%.1fs)", total, time.time() - t0)
        return results
    ...
    def _nmap_scan(self, targets: list[str]) -> dict:
        ...
        total = sum(len(v) for v in results.values())
        logger.info("Nmap scan completed : %d ports ouverts (%.1fs)", total, time.time() - t0)
        return results
    ...
    def _grab_banner(ip: str, port: int, timeout: float = 2.0) -> tuple[str, str]:
        """Attempts to retrieve the banner d'un service TCP."""
        try:
    ...
    def _update_hosts(self, hosts: list[dict]) -> int:
        ...
        with db.atomic():
            for h in hosts:
                ...
                # Chercher d'abord par MAC (plus reliable que par IP en DHCP)
                obj = None
                if mac:
    ...
    def _update_ports(self, ip: str, ports: list[dict]):
            ...
            for p in ports:
                ...
                if created:
                    self._alert(severity=3, source="discovery", category="new_port",
                                title=f"New port {p['port']}/tcp sur {ip}",
        ...
        if self.identity and ports:
            ...
            if mac:
                ...
                for p in ports:
                    if p.get("banner"):
                        banners[str(p["port"])] = hashlib.md5(
