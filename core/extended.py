...
logger = logging.getLogger("cgs.extended")
...
class HashChainAudit:
    ...
    def __init__(self, config):
        self.log_dir = config.get("general.log_dir", "/var/log/cgs")
    ...
    def _resume(self):
        ...
        try:
            ...
            logger.debug("Failed to resume audit chain: %s", e)
...
class BackupManager:
    ...
    def __init__(self, config):
        self.cfg = config
        self.data_dir = config.get("general.data_dir", "/var/lib/cgs/data")
        self.log_dir = config.get("general.log_dir", "/var/log/cgs")
        self.config_path = config.get("_config_path", "/etc/cgs/config.yaml")
    ...
    def create(self, passphrase: str = "") -> str:
        ...
        tar_path = os.path.join(self.backup_dir, f"cgs_backup_{now}.tar.gz")

        with tarfile.open(tar_path, "w:gz") as tar:
            # Database
            db_path = os.path.join(self.data_dir, "cgs.db")
            if os.path.exists(db_path):
                tar.add(db_path, arcname="data/cgs.db")
    ...
    def _watch_loop(self):
        ...
        while True:
            ...
            try:
                ...
                logger.debug("Failed to check rules file for changes: %s", e)
...
<html><head><meta charset="utf-8"><title>CGS Weekly Report</title></head>
<body style="font-family:Arial;max-width:700px;margin:20px auto;padding:20px">
<h1 style="color:#1F2937">CGS — Weekly Report</h1>
...
CGS v2.2.3 — Automated weekly report</p>
</body></html>"""
...
class ThreatIntel:
    ...
    def __init__(self, config):
        self.misp_url = self._validate_url(config.get("threat_intel.misp_url", ""))
        self.misp_key = config.get("threat_intel.misp_key", "")
        self.opencti_url = self._validate_url(config.get("threat_intel.opencti_url", ""))
    ...
    def _validate_url(url: str) -> str:
        """Only allow http(s) schemes to prevent file:/ or other dangerous schemes."""
        if not url:
            return ""
        if not url.startswith(("https://", "http://")):
            logger.warning("Threat intel URL rejected (not http/https): %s", url[:40])
            return ""
        return url
    ...
    def _query_misp(self, ip: str) -> dict:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:  # nosec B310 — scheme validated in _validate_url
...
class FalsePositiveManager:
    ...
    def _load(self):
        try:
            ...
            logger.debug("Failed to load false positive data: %s", e)
            self._data = {}
    ...
    def _save(self):
        try:
            ...
            logger.debug("Failed to save false positive data: %s", e)
