"""
CGS — Forensic evidence collection.

Generates a structured JSON file containing ALL information
related to an incident for future forensic analysis:

  - Complete timeline
  - Raw Suricata alerts
  - Attacker reconnaissance results
  - Target host state (ports, services, risque)
  - Associated network flows (from the DB)
  - Suspicious DNS queries
  - Executed defense actions
  - Complete audit trail
  - Sentinel metadata

Each file is timestamped and named by the incident ID.
"""
...
logger = logging.getLogger("cgs.forensic")


class ForensicCollector:
    """Collects and saves forensic evidence for an incident."""

    def __init__(self, config):
        self.cfg = config
        self.output_dir = os.path.join(
            config.get("general.log_dir", "/var/log/cgs"),
            "forensics"
        )
    ...
    def collect_and_save(
        ...
        """
        Collects all evidence and saves it to a JSON file.
            ...
            try:
                ...
                if not target_mac:
                    try:
                        ...
                        logger.debug("Failed to resolve target MAC for forensic: %s", e)
                if target_mac:
                ...
                logger.debug("Forensic fingerprint: %s", e)
        ...
        evidence = {
            "_metadata": {
                ...
                "note": (
                    "This file contains digital evidence related to a security "
        ...
        logger.warning("📁 Forensic evidence saved: %s (%.1f KB)", filepath, size_kb)
    ...
    def _build_timeline(incident_data: dict, created_at: float) -> list[dict]:
        ...
        if incident_data.get("resolved"):
            events.append({"timestamp": "", "event": f"Resolution : {incident_data.get('resolution', '')}"})

        if incident_data.get("report_sent"):
            events.append({"timestamp": "", "event": "Report email sent"})
    ...
    def _get_host_info(ip: str) -> dict:
        try:
            ...
            logger.debug("Failed to get host info for forensic: %s", e)
        return {}
