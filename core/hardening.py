"""
CGS — Advanced security hardening.
...
logger = logging.getLogger("cgs.hardening")
...
class TLSAutoGen:
    ...
    is the default, not an option. Cert stored in /etc/cgs/tls/.
    """

    CERT_DIR = "/etc/cgs/tls"
    ...
    def ensure_cert(cls, config) -> tuple[str, str]:
        ...
        try:
            ...
            logger.debug("Failed to get hostname for TLS cert: %s", e)
        ...
        try:
            subprocess.run([
                ...
                "-days", "3650", "-nodes",
                "-subj", f"/CN={hostname}/O=CGS/OU=Auto-Generated",
...
class IntegrityCheck:
    ...
    in /var/lib/cgs/data/integrity_manifest.json.
    """

    INSTALL_DIR = "/opt/cgs"
    MANIFEST_PATH = "/var/lib/cgs/data/integrity_manifest.json"
...
class FirewallVerifier:
    ...
    def snapshot_expected(self):
        """Capture current CGS rules as the expected state."""
    ...
    def verify(self) -> dict:
        ...
        if missing:
            logger.warning("FIREWALL TAMPERED: %d/%d CGS rules missing!",
    ...
    def _get_current_rules(self) -> list[str]:
        """Get current CGS iptables rules."""
        try:
            r = subprocess.run(
                ["iptables", "-L", "CGS", "-n", "--line-numbers"],
            ...
            logger.debug("Failed to get current firewall rules: %s", e)
            return []
        ...
        for rule in missing_rules:
            ...
            if len(parts) >= 5 and parts[1] in ("DROP", "REJECT"):
                ...
                if src:
                    try:
                        subprocess.run(
                            ["iptables", "-A", "CGS", "-s", src, "-j", parts[1]],
                            capture_output=True, timeout=5)
                        reapplied += 1
                    except Exception as e:
                        logger.warning("Failed to re-apply firewall rule for %s: %s", src, e)
...
class SSHHardener:
    ...
    def _read_sshd_config(self) -> str:
        ...
        if os.path.isdir(self.SSHD_CONFIG_DIR):
            try:
                ...
                logger.debug("Failed to read sshd_config.d: %s", e)
    ...
    def _get_effective_value(self, config: str, directive: str) -> str:
        ...
        sshd uses first-match-wins, so we return the first uncommented occurrence.
        Note: sshd_config.d/ files are typically included at the TOP of sshd_config
        on Ubuntu 22.04+ (via 'Include /etc/ssh/sshd_config.d/*.conf'),
        so they appear first in our concatenated config and take precedence correctly.
        """
        import re
        ...
        if matches:
            return matches[0].lower()  # First match wins (sshd semantics)
        return ""
        ...
        if os.path.isdir(self.SSHD_CONFIG_DIR):
            for fn in os.listdir(self.SSHD_CONFIG_DIR):
                ...
                try:
                    ...
                    logger.debug("Failed to update sshd_config.d/%s: %s", fn, e)
