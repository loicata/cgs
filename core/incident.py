"""
CyberGuard Sentinel — Incident response engine.
...
logger = logging.getLogger("cyberguard.incident")
  ...
  <div style="padding:24px 30px">
    <div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:8px;padding:16px;margin-bottom:20px">
      ...
      <p style="margin:10px 0 0;color:#DC2626;font-size:13px">
        ...
        puis le Sentinel attendra l'extinction effective avant d'agir.
      </p>
    </div>
    ...
    <p style="color:#BBB;font-size:11px;margin:16px 0 0">— CyberGuard Sentinel</p>
  </div>
</div></body></html>"""
...
<div style="max-width:600px;margin:20px auto;background:white;border-radius:12px;overflow:hidden;
  ...
  <div style="padding:24px 30px">
    ...
    <p style="color:#999;font-size:11px;margin:16px 0 0">
      — CyberGuard Sentinel · Automated message
    </p>
  </div>
</div></body></html>"""
...
<div style="max-width:600px;margin:20px auto;background:white;border-radius:12px;overflow:hidden;
  ...
  <div style="padding:24px 30px">
    ...
    {reco_html}
    <p style="color:#999;font-size:11px;margin-top:16px">— CyberGuard Sentinel · {report_time}</p>
  </div>
</div></body></html>"""
...
class IncidentResponseEngine:
    ...
    def __init__(self, config, alert_fn, defense_engine, mac_resolver=None):
        ...
        self.from_addr = config.get("email.from_address", "sentinel@cyberguard.local")
    ...
    def _execute_and_report(self, inc: Incident):
        ...
        # ═══════════════════════════════════════════
        # PHASE 1 : Reconnaissance approfondie de l'attaquant
        # ═══════════════════════════════════════════
        recon_report = {}
        if inc.attacker_ip:
        ...
        for ioc in inc.iocs:
            if "." in ioc and not ioc.replace(".", "").isdigit():
                ...
                if not already:
                    ...
                    executed.append(f"✓ DNS sinkhole (recon) sur {ioc}")
            ...
            try:
                ...
                complaint_pdf_path = generate_complaint_pdf(
                    ...
                    output_dir=os.path.join(
                        self.cfg.get("general.log_dir", "/var/log/cyberguard"),
                        "forensics"
                    ),
    ...
    def _plan(self, inc: Incident) -> list[str]:
        ...
        for ioc in inc.iocs:
            if "." in ioc and not ioc.replace(".", "").isdigit():
                actions.append(f"DNS sinkhole sur {ioc}")
    ...
    def _send_admin_email(self, inc: Incident, reminder=False):
        ...
        subject = f"{'🔁 REMINDER — ' if reminder else ''}⚠️ [CyberGuard] {inc.id} — Approval required"
    ...
    def _send_user_shutdown_email(self, inc: Incident):
        ...
        self._smtp([inc.target_email],
                   "🔴 [CyberGuard] Security alert — Please shut down your computer",
                   html)
    ...
    def _send_report(self, inc: Incident, recon_report: dict = None, forensic_path: str = "",
        ...
        if recon_report and recon_report.get("target_ip"):
            ...
            <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:14px;font-size:12px;margin-bottom:16px">
              <table style="width:100%;border-collapse:collapse">
                ...
                <tr><td style="padding:3px 0;color:#666">Proxy/VPN/Tor</td><td>{'Oui ⚠️' if geo.get('proxy') or geo.get('hosting') else 'Not detected'}</td></tr>
        ...
        if attached_files:
            ...
            forensic_html = f"""<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;
                ...
                Keep these files safely — ils constituent des digital evidence
                pour un potential complaint filing.
                </span>
            </div>"""
        elif forensic_path:
            forensic_html = f"""<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:8px;
        ...
        <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:16px;font-size:12px;margin-bottom:16px">
          ...
          <table style="width:100%;border-collapse:collapse;margin-bottom:10px">
            ...
            if police.get("url_online"):
                label = "Online report" if self.country_code == "US" else "Plainte en ligne"
                legal_html += f"""
        ...
        if inc.snapshot_path:
            ...
            <div style="background:#FEF3C7;border:1px solid #FDE68A;border-radius:8px;
              ...
              <ul style="padding-left:18px;margin:0;color:#374151;line-height:1.8">
                <li>SSH: <code>sudo cyberguard console</code> → Active defense → Rollback</li>
        ...
        if self.defense_mode == "immediate":
            subject = f"{'✅' if is_ok else '⚠️'} [CyberGuard] {inc.id} — {'Threat neutralized' if is_ok else 'Action taken — review needed'}"
        else:
            subject = f"{'✅' if is_ok else '⚠️'} [CyberGuard] Report {inc.id} — {v[5]}"
