...
lease expiration, etc.). Sentinel cannot rely on IPs alone.
...
This module:
  ...
  - Automatically updates:
    ...
    · The whitelist
  - Alert (info) when an IP changes, without false positive "new host"
"""
...
logger = logging.getLogger("cgs.macresolver")


class MacIpResolver:
    """Dynamic MAC ↔ IP table with change propagation."""
    ...
    def __init__(self, config, alert_fn):
        ...
        # Registered callbacks by other modules when an IP changes
    ...
    def update(self, mac: str, ip: str, hostname: str = "", vendor: str = "",
        ...
        with self._lock:
            ...
            # If another MAC had this IP before → conflict (rare but possible)
            for other_mac, other_entry in self._mac_table.items():
                if other_mac != mac and other_entry["ip"] == ip:
                    logger.warning(
                        "⚠ IP conflict: %s assigned to MAC %s AND %s",
                        ip, mac, other_mac
                    )
    ...
    def mac_to_ip(self, mac: str) -> str:
        """Returns the current IP for a MAC."""
        with self._lock:
    ...
    def get_user_email(self, ip: str) -> dict:
        ...
        Searches first by IP in the directory, then by MAC.
        """
    ...
    def _handle_ip_change(self, mac: str, old_ip: str, new_ip: str):
        ...
        try:
            ...
            logger.error("DB IP change error : %s", e)
    ...
    def _load_from_db(self):
        ...
        try:
            ...
            logger.warning("Failed to load MAC table from DB: %s", e)
