"""
CGS — Netgate firewall control (pfSense / OPNsense).

Allows Sentinel to act directly on the perimeter firewall:
  - Add/remove IPs in a block alias
  - Create dynamic block rules
  - Apply changes (reload firewall)
  - Read firewall state (interfaces, rules, aliases)

Supports both Netgate firmware:
  - pfSense  : via the pfSense-pkg-API package (REST) or FauxAPI
  - OPNsense : via native REST API (key + secret)

The main approach uses an alias (address table) named
"CGS_Block" that Sentinel fills dynamically.
The admin just needs to create a block rule pointing to this alias.
"""
...
logger = logging.getLogger("cgs.netgate")


class _SuppressInsecureWarnings:
    """Context manager to suppress SSL warnings only for Netgate API calls."""
    def __enter__(self):
...
class NetgateFirewall:
    ...
    def __init__(self, config):
        ...
        self.fw_type = config.get("netgate.type", "").lower()  # "pfsense" or "opnsense"
        ...
        self.block_alias = config.get("netgate.block_alias", "CGS_Block")

        # Auth pfSense (API key + client ID or user/password for FauxAPI)
        ...
        # Cache of currently blocked IPs on the firewall
    ...
    def block_ip(self, ip: str, reason: str = "") -> bool:
        ...
        logger.warning("🔥 Netgate : blocking %s on %s (%s)", ip, self.host, reason)
    ...
    def unblock_ip(self, ip: str) -> bool:
        """Removes an IP from the block alias."""
        if not self.enabled:
            return False
    ...
    def get_blocked_ips(self) -> list[str]:
        """Returns the list of IPs in the block alias."""
        self._sync_blocked()
    ...
    def _pf_request(self, method: str, endpoint: str, data: dict = None) -> Optional[dict]:
        ...
        try:
            ...
            logger.error("pfSense API error : %s", e)
            return None
    ...
    def _pf_add_to_alias(self, ip: str) -> bool:
        ...
        result = self._pf_request("POST", "/firewall/alias/entry", {
            ...
            "address": [ip],
            "detail": [f"CGS {datetime.now().strftime('%Y-%m-%d %H:%M')}"],
        })
    ...
    def _opn_add_to_alias(self, ip: str) -> bool:
        """Adds an IP to an OPNsense alias via the firewall API."""
        # Retrieve alias UUID
        alias_uuid = self._opn_get_alias_uuid()
        if not alias_uuid:
            logger.error("Alias '%s' not found on OPNsense. Create it in Firewall > Aliases.",
                        self.block_alias)
            return False

        # Retrieve current alias content
        ...
        # Fallback: modify the alias directly
    ...
    def _sync_blocked(self):
        """Synchronizes local list with the alias on the firewall."""
        if not self.enabled:
            return
        now = time.time()
            ...
            if self.fw_type == "pfsense":
                ...
                if data and "rows" in data:
                    for row in data["rows"]:
                        ...
                        if ip:
                            self._blocked_ips.add(ip)
            logger.debug("Sync Netgate: %d IPs in alias", len(self._blocked_ips))
        except Exception as e:
            logger.warning("Sync Netgate failed: %s", e)
