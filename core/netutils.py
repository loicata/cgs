"""
CyberGuard Sentinel — Network utilities (scapy).
...
Additional utility functions (subnet, entropie, OUI, OS guess).
"""
...
def guess_os_from_ttl(ttl: int) -> str:
    """Estimates OS from TTL value d'un paquet ICMP/IP."""
...
def get_all_interfaces() -> dict:
    ...
    try:
        for iface in os.listdir("/sys/class/net"):
            ...
            try:
                info["up"] = open(f"/sys/class/net/{iface}/operstate").read().strip() == "up"
            except Exception:
                pass
            try:
                info["speed"] = int(open(f"/sys/class/net/{iface}/speed").read().strip())
            except Exception:
                pass
            result[iface] = info
    except Exception:
