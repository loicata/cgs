"""
CyberGuard Sentinel — OS hardening module.
...
Drop-in files created (all removable with one command):
  /etc/sysctl.d/90-cyberguard.conf
  /etc/modprobe.d/cyberguard-blacklist.conf
  /etc/security/limits.d/cyberguard.conf
  /etc/apparmor.d/opt.cyberguard.daemon (if AppArmor available)
  /etc/audit/rules.d/cyberguard.rules (if auditd available)

To undo ALL hardening:
  rm /etc/sysctl.d/90-cyberguard.conf
  rm /etc/modprobe.d/cyberguard-blacklist.conf
  rm /etc/security/limits.d/cyberguard.conf
  rm /etc/apparmor.d/opt.cyberguard.daemon
  rm /etc/audit/rules.d/cyberguard.rules
  sysctl --system
"""
...
logger = logging.getLogger("cyberguard.os_hardening")
...
class OSHardener:
    ...
    SYSCTL_FILE = "/etc/sysctl.d/90-cyberguard.conf"
    MODPROBE_FILE = "/etc/modprobe.d/cyberguard-blacklist.conf"
    LIMITS_FILE = "/etc/security/limits.d/cyberguard.conf"
    APPARMOR_FILE = "/etc/apparmor.d/opt.cyberguard.daemon"
    AUDITD_FILE = "/etc/audit/rules.d/cyberguard.rules"
    BANNER_FILE = "/etc/issue.net"
    TMOUT_FILE = "/etc/profile.d/cyberguard-tmout.sh"
    ...
    def undo(self) -> list:
        """Remove all CyberGuard OS hardening (clean uninstall)."""
        removed = []
    ...
    def _harden_sysctl(self) -> list:
        ...
        changes = []
        lines = ["# CyberGuard Sentinel — Kernel hardening",
    ...
    def _harden_mounts(self) -> list:
        ...
        try:
            ...
            if needed:
                ...
                subprocess.run(
                    ["mount", "-o", f"remount,{new_opts}", "/tmp"],
                    capture_output=True, timeout=10)
                changes.append(f"/tmp remounted with {','.join(needed)}")
                logger.info("/tmp hardened: added %s", ",".join(needed))
            else:
                changes.append("/tmp already has noexec,nosuid,nodev")

        except Exception as e:
            changes.append(f"/tmp mount check skipped: {e}")
    ...
    def _harden_modules(self) -> list:
        ...
        lines = ["# CyberGuard Sentinel — Module blacklist",
                 "# Auto-generated, safe to delete",
                 ""]
    ...
    def _harden_apparmor(self) -> list:
        ...
        profile = """# CyberGuard Sentinel — AppArmor profile
# Auto-generated, safe to delete

/opt/cyberguard/venv/bin/python3 {
  # Python interpreter
  /opt/cyberguard/** r,
  /opt/cyberguard/venv/** rix,
  /usr/lib/python3/** r,
  /usr/lib/python3.*/** r,
  ...
  # Data directories
  /var/lib/cyberguard/** rw,
  /var/log/cyberguard/** rw,
...
"""
        ...
        try:
            ...
            changes.append("Run 'aa-enforce /etc/apparmor.d/opt.cyberguard.daemon' "
    ...
    def _harden_auditd(self) -> list:
        ...
        rules = """# CyberGuard Sentinel — Audit rules
# Auto-generated, safe to delete

# Monitor config changes
-w /etc/cyberguard/ -p wa -k cyberguard_config
...
-w /opt/cyberguard/ -p wa -k sentinel_code
"""
    ...
    def _harden_services(self) -> list:
        ...
        for service, reason in unnecessary:
            try:
                ...
                pass  # Service doesn't exist — fine
    ...
    def _harden_tmout(self) -> list:
        ...
        script = """# CyberGuard Sentinel — Shell timeout
# Auto-generated, safe to delete
