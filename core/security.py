"""
CyberGuard Sentinel — Security hardening module.
...
class SecretsVault:
    ...
    def _init_fernet(self, passphrase: str):
        try:
            ...
            salt = b"cyberguard-sentinel-v2"  # Fixed salt (per-install would be better)
...
def generate_agent_manifest(agent_path: str, secret: str) -> dict:
    ...
    return {
        ...
        "verify_command": f'echo "{checksum}  cyberguard-agent.py" | sha256sum -c -',
    }
...
def harden_permissions():
    ...
    paths = {
        "/etc/cyberguard": 0o750,
        "/etc/cyberguard/config.yaml": 0o640,
        "/var/lib/cyberguard/data": 0o750,
        "/var/log/cyberguard": 0o750,
        "/var/log/cyberguard/snapshots": 0o700,
        "/var/log/cyberguard/forensics": 0o700,
    }
...
def drop_privileges(uid_name: str = "cyberguard"):
    """
