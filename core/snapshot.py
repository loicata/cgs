"""
CGS — Defense state snapshot & rollback.
...
Snapshots are stored as JSON files in /var/log/cgs/snapshots/.
"""
...
logger = logging.getLogger("cgs.snapshot")
...
class DefenseSnapshot:
    ...
    def take(self, incident_id: str, reason: str = "") -> str:
        ...
        snapshot = {
            ...
            # CGS-specific state
            "cgs_chain": self._capture_cgs_chain(),
        }
    ...
    def restore(self, snapshot_path: str) -> dict:
        ...
        # 4. Flush CGS iptables chain
        if snapshot.get("cgs_chain"):
            ok = self._restore_cgs_chain(snapshot["cgs_chain"])
            if ok:
                actions.append("CGS iptables chain restored")
            else:
                errors.append("CGS chain restore failed")
    ...
    def list_snapshots(self) -> list[dict]:
        ...
        for filename in sorted(os.listdir(self.snapshot_dir), reverse=True):
            ...
            try:
                ...
                logger.debug("Failed to read snapshot %s: %s", filepath, e)
                continue
        return snapshots
    ...
    def _capture_cgs_chain(self) -> str:
        """Capture CGS-specific iptables chain."""
        try:
            result = subprocess.run(
                ["iptables", "-L", "CGS", "-n", "-v", "--line-numbers"],
    ...
    def _restore_cgs_chain(self, saved_state: str) -> bool:
        """Restore CGS iptables chain to previous state."""
        try:
            # Flush the chain
            subprocess.run(
                ["iptables", "-F", "CGS"],
            ...
            if saved_state and "CGS" in saved_state:
                for line in saved_state.strip().split("\n"):
                    ...
                    if len(parts) >= 4 and parts[1] in ("DROP", "REJECT"):
                        ...
                        if src:
                            subprocess.run(
                                ["iptables", "-A", "CGS", "-s", src, "-j", target],
                                capture_output=True, timeout=5)
            return True
        except Exception as e:
            logger.error("CGS chain restore: %s", e)
            return False
