"""
CyberGuard Sentinel — Ingestion Suricata EVE JSON.

Three reception modes:
  1. Tail eve.json file (if NFS/sshfs mounted from firewall)
  2. Syslog UDP/TCP (le firewall envoie ses logs ici)
  3. HTTP webhook (le firewall POST les alertes)
...
logger = logging.getLogger("cyberguard.suricata")
...
class SuricataIngester:
    ...
    def start(self):
        ...
        if eve_path and os.path.exists(eve_path):
            ...
            logger.info("Ingestion Suricata : fichier %s", eve_path)
        ...
        if syslog_port:
            ...
            t.start()
            self._threads.append(t)
            logger.info("Ingestion Suricata : syslog UDP :%d", syslog_port)
        ...
        if tcp_port:
            ...
            t.start()
            self._threads.append(t)
            logger.info("Ingestion Suricata : TCP :%d", tcp_port)

        if not self._threads:
            logger.warning("None canal Suricata configured. "
                           "Configure suricata.eve_file, suricata.syslog_port ou suricata.tcp_port")
            return
        ...
        try:
            ...
            while not self._stop.is_set():
                try:
                    ...
                    # Syslog peut avoir un header — trouver le JSON
        ...
        try:
            ...
            while not self._stop.is_set():
                try:
                    ...
                    logger.info("Login TCP depuis %s", addr)
                    t = threading.Thread(
