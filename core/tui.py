"""
CGS — TUI administration console (SSH).

Accessible via: sudo cgs console

Main menu:
  1. System status (services, CPU, RAM, disk)
  2. Recent alerts
  3. Network inventory
...
logger = logging.getLogger("cgs.tui")
...
def run_console(config_path: str = "/etc/cgs/config.yaml"):
    ...
    while True:
        console.clear()
        console.print(Panel(
            "[bold cyan]⛨  CGS — Administration console[/]\n"
        ...
        menu = [
            ...
            ("5", "Active defense actions", _show_defense),
...
def _show_status(cfg, config_path):
    ...
    t.add_row("RAM", f"{s['memory_percent']}% ({s['mem_used_gb']}/{s['mem_total_gb']} GB)")
    t.add_row("Uptime", f"{s['uptime_h']}h")
    for mount, d in r["disk"].items():
        c = "green" if d["used_pct"] < 75 else "yellow" if d["used_pct"] < 90 else "red"
        t.add_row(f"Disk {mount}", f"[{c}]{d['used_pct']}%[/] ({d['free_gb']} GB free)")
    console.print(t)

    # Services
    services = ["cgs"]
    # Add Suricata only if it is configured
...
def _show_inventory(cfg, config_path):
    ...
    try:
        ...
        logger.debug("Failed to load identity engine for inventory: %s", e)
...
def _show_incidents(cfg, config_path):
    ...
    try:
        ...
        cert_path = cfg.get("web.ssl_cert", "/etc/cgs/tls/sentinel.crt")
        r = requests.get(f"https://localhost:{port}/api/incidents",
                        verify=cert_path if os.path.exists(cert_path) else False,  # nosec B501 — local self-signed cert
                        timeout=5,
        ...
        console.print("[dim]  Is the service running? sudo systemctl start cgs[/]")
...
def _show_defense(cfg, config_path):
    ...
    try:
        ...
        if r.returncode == 0 and r.stdout.strip():
            console.print("[bold]CGS chain (iptables):[/]")
            console.print(r.stdout)
        else:
            console.print("  No active CGS iptables rules.")
    except Exception:
...
def _edit_config(cfg, config_path):
    import yaml
    console.print("\n[bold cyan]  Modifying configuration[/]\n")
    choices = [
    ...
    if choice == "1":
        ...
        if Confirm.ask("Restart service to apply?", default=True):
        ...
        console.print(f"  Attach forensic file              : [{'green' if forensic else 'red'}]{'Enabled' if forensic else 'Disabled'}[/]")
        console.print()

        if Confirm.ask("  Modify these options?", default=True):
            # Country selection
...
def _manage_users(cfg, config_path):
    ...
    while True:
        ...
        if choice == "0":
            ...
            # Index existing entries by IP and MAC
            ...
            for host in discovered:
                ...
                name = Prompt.ask("  User name (empty=skip)", default="")
                if name:
            ...
            if new_count > 0:
                _save_users()
                console.print(f"\n  [green]{new_count} new user(s) added[/]")
            else:
...
def _test_email(cfg, config_path):
    ...
    try:
        ...
        msg = MIMEText("This is a test email from CGS.\n\nIf you receive this message, the SMTP configuration is correct.")
...
def _view_logs(cfg, config_path):
    console.print("\n[bold cyan]  Logs[/]\n")
    console.print("  1. CGS logs (journalctl)")
    console.print("  2. cgs.log file")
    ...
    if choice == "1":
        subprocess.run(["journalctl", "-u", "cgs", "--no-pager", "-n", "50"])
    elif choice == "2":
        log_file = os.path.join(cfg.get("general.log_dir", "/var/log/cgs"), "cgs.log")
        ...
        if os.path.exists(audit_file) and os.path.isfile(audit_file):
            subprocess.run(["tail", "-30", audit_file])
        else:
...
def _restart_service(cfg, config_path):
    ...
    r = subprocess.run(["systemctl", "restart", "cgs"], capture_output=True)
    if r.returncode == 0:
        console.print("  [green]✓ Service restarted.[/]")
    else:
        console.print("  [red]✗ Restart failed. See: journalctl -u cgs[/]")
