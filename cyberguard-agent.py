#!/usr/bin/env python3
"""
CyberGuard Sentinel — Secure Client Agent (runs on workstations).
...
Usage:
  python3 cyberguard-agent.py --server https://192.168.1.100:8443 --secret MY_SHARED_KEY
  python3 cyberguard-agent.py --server https://sentinel:8443 --secret MY_KEY --no-verify-ssl
...
log = logging.getLogger("cyberguard-agent")
...
MESSAGES = {
    "shutdown": {
        "title": "SECURITY ALERT — CyberGuard Sentinel",
        "body": (
    ...
    "all_clear": {
        "title": "Threat resolved — CyberGuard Sentinel",
        "body": (
    ...
    "risk_warning": {
        "title": "WARNING — Do NOT restart — CyberGuard Sentinel",
        "body": (
    ...
    "collect_forensic": {
        "title": "Security scan request — CyberGuard Sentinel",
        "body": (
    ...
    "collect_done": {
        "title": "Scan complete — CyberGuard Sentinel",
        "body": (
...
def collect_local_forensics(config: AgentConfig, incident_id: str) -> str:
    ...
    filename = (f"CyberGuard_Forensic_{incident_id}_"
...
def main():
    parser = argparse.ArgumentParser(
        description="CyberGuard Sentinel Client Agent — secure, zero-privilege",
        ...
        epilog=(
            ...
            "  Forensic reports are saved to Desktop only (never sent anywhere)\n\n"
            "Examples:\n"
            "  python3 cyberguard-agent.py --server https://192.168.1.100:8443 --secret MyKey123\n"))
    ...
    parser.add_argument("--version", action="version", version=f"cyberguard-agent {__version__}")
    args = parser.parse_args()

    config = AgentConfig(args)
    client = SentinelClient(config)

    log.info("CyberGuard Agent v%s started", __version__)
