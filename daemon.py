"""CyberGuard Sentinel — Daemon with active defense."""
...
class Daemon:
    def __init__(self, cfg_path=None):
        ...
        self._log_setup()
        self.log = logging.getLogger("cyberguard.daemon")
        self.log.info("=" * 50)
        self.log.info("  CyberGuard Sentinel v2.2.3 - Active Defense")
        self.log.info("=" * 50)
        ...
        self.sniffer = PacketSniffer(self.cfg, self.engine.on_event, self.identity)
        ...
        db_path = os.path.join(self.cfg.get("general.data_dir", "/var/lib/cyberguard/data"), "cyberguard.db")
    ...
    def _log_setup(self):
        ld = self.cfg.get("general.log_dir"); os.makedirs(ld, exist_ok=True)
        lv = getattr(logging, self.cfg.get("general.log_level","INFO").upper(), logging.INFO)
        from core.security import SanitizedFormatter
        fmt = SanitizedFormatter("%(asctime)s │ %(levelname)-8s │ %(name)-24s │ %(message)s", datefmt="%H:%M:%S")
        sh = logging.StreamHandler(sys.stdout); sh.setFormatter(fmt)
        fh = logging.FileHandler(os.path.join(ld, "cyberguard.log"), encoding="utf-8"); fh.setFormatter(fmt)
        r = logging.getLogger("cyberguard"); r.setLevel(lv); r.addHandler(sh); r.addHandler(fh)
    ...
    def start(self):
        ...
        drop_privileges(self.cfg.get("general.run_as_user", "cyberguard"))
        ...
        if self.cfg.get("web.enabled", True): self._start_web()
        modes = []
        if self.cfg.get("suricata.eve_file"): modes.append(f"eve:{self.cfg.get('suricata.eve_file')}")
        if self.cfg.get("suricata.syslog_port"): modes.append(f"syslog:{self.cfg.get('suricata.syslog_port')}")
        if self.cfg.get("suricata.tcp_port"): modes.append(f"tcp:{self.cfg.get('suricata.tcp_port')}")
        ...
        while self._running:
            ...
            except Exception as e: self.log.error("Scheduler: %s", e); time.sleep(5)

    def _safe(self, name, fn, *a):
        def r():
            try: fn(*a)
            except Exception as e: self.log.error("[%s] %s", name, e, exc_info=True)
    ...
    def _start_web(self):
        ...
        h, p = self.cfg.get("web.host","0.0.0.0"), self.cfg.get("web.port",8443)
        def run():
            ctx = None
    ...
    def _cleanup(self):
        now = datetime.now()
        with db.atomic():
            a=Alert.delete().where(Alert.ts<now-timedelta(days=self.cfg.get("retention.alerts_days",90))).execute()
            f=Flow.delete().where(Flow.ts<now-timedelta(days=self.cfg.get("retention.flows_days",14))).execute()
            d=DnsLog.delete().where(DnsLog.ts<now-timedelta(days=self.cfg.get("retention.events_days",30))).execute()
        if a or f or d: self.log.info("Purge: %d alerts, %d flows, %d DNS", a, f, d)
    ...
    def _send_weekly_report(self):
        ...
        try:
            ...
            if admins and self.cfg.get("email.enabled"):
                self.incident._smtp(admins, "[CyberGuard] Weekly Security Report", html)
